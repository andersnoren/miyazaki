...
class Miyazaki_Recent_Comments extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' 	=> 'widget_miyazaki_recent_comments',
			'description' 	=> __( 'Displays recent comments with user avatars.', 'miyazaki' ),
		);
		parent::__construct( 'widget_miyazaki_recent_comments', __( 'Recent Comments', 'miyazaki' ), $widget_ops );
	}
				...
				if ( $comments ) {
					...
					foreach ( $comments as $comment ) { ?>
						...
						<li>
							...
							<a href="<?php echo esc_url( get_permalink( $comment->comment_post_ID ) ); ?>#comment-<?php echo esc_attr( $comment->comment_ID ); ?>" title="<?php printf( _x( 'Comment to %1$s, posted %2$s', 'Translators: %1$s = post title, %2$s = post date', 'miyazaki' ), esc_attr( get_the_title( $comment->comment_post_ID ) ), esc_attr( get_the_time( get_option( 'date_format' ) ) ) ); ?>">
								...
								<div class="inner">
									...
									<p><?php echo wp_kses_post( get_comment_date( get_option( 'date_format' ) ) ); ?></p>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'widget_title' ) ); ?>"><?php  _e( 'Title', 'miyazaki' ); ?>:
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number_of_comments' ); ?>"><?php _e( 'Number of comments to display', 'miyazaki' ); ?>:
